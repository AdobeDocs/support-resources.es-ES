...
| [Conversor de tablas avanzado](https://tableconvert.com/html-to-markdown) | Convierta tablas de cualquier formato a cualquier formato. <p>**Nota:** Los vínculos y las imágenes se acoplan al convertirlos. |
| [Tabla básica html > Conversor de markdown](https://jmalarcon.github.io/markdowntables/) | Convertidor de HTML simple <p>**Nota:** Los vínculos y las imágenes se acoplan al convertirlos. |
| [HTML que no es de tabla > Conversor de Markdown](https://codebeautify.org/html-to-markdown) | Convierte las tablas de HTML en sintaxis Markdown no de tabla. Se utiliza en combinación con las herramientas anteriores para copiar vínculos, imágenes y cualquier otro elemento aplanado. |
...
Para facilitar la migración, hemos ampliado las tablas de Markdown para que admitan saltos de párrafo de HTML (`<p>`), saltos de línea (`<br>`) y listas de HTML básicas (`<ul>` y `<ol>`) dentro de las tablas de Markdown.
...
| Salto de línea | primera línea en la celda<br>segunda línea en la celda | row 1 column 3 |
...
**Tabla de HTML migrada de ejemplo**
...
* No utilice la sintaxis Markdown en tablas de HTML. Por ejemplo, si agrega `[!NOTE]` a una tabla de HTML, se procesará tal cual (`[!NOTE]`). En su lugar, utilice la sintaxis del HTML para elementos como notas e imágenes.
...
* Para alinear el texto, use `align: "left|center|right"` en el HTML. Por ejemplo, para centrar el contenido de una celda de tabla, use `<td align="center">`.
...
* **Fijo** (actualmente el valor predeterminado): incluye reglas personalizadas para representar tablas, incluidas tablas de HTML con imágenes. Las tablas se representan como tablas de ancho completo sin desplazamiento, lo que a veces provoca que el texto se superponga.
* **Automático**: similar a Markdown de Git (GFM). Las tablas pueden desplazarse, por lo que el texto no se superpone.

En la mayoría de los casos, las tablas se representan con el mismo aspecto. Sin embargo, si la tabla incluye texto superpuesto, deseará aplicar la etiqueta `auto`. O bien, si la tabla de HTML con tarjetas de imagen no se está representando correctamente, puede que desee aplicar la etiqueta `fixed`.

Estamos considerando cambiar el valor predeterminado de `fixed` a `auto`.
...
![procesamiento de tabla](assets/table-render.png)
...
Utilice `auto` para tablas con bloques de código largos o texto que ocasione texto superpuesto cuando se selecciona `fixed` (predeterminado).

*Fijo (Predeterminado)*

| Métrica de perspectivas | Descripción | Parámetro de consulta de ID |
| ---- | ---- | ---- |
| **series de tiempo.data.collection.validation.category.type.count** | Número total de mensajes de &quot;tipo&quot; no válidos para un conjunto de datos o para todos ellos. | ID de conjunto de datos |
| **series de tiempo.data.collection.validation.category.range.count** | Número total de mensajes de &quot;intervalo&quot; no válidos para un conjunto de datos o para todos ellos. | ID de conjunto de datos |
| **series de tiempo.data.collection.validation.category.format.count** | Número total de mensajes de &quot;formato&quot; no válidos para un conjunto de datos o para todos ellos. | ID de conjunto de datos |
| **series de tiempo.data.collection.validation.category.pattern.count** | Número total de mensajes de &quot;patrón&quot; no válidos para un conjunto de datos o para todos ellos. | ID de conjunto de datos |
...
| **series de tiempo.data.collection.validation.category.enum.count** | Número total de mensajes &quot;enum&quot; no válidos para un conjunto de datos o para todos ellos. | ID de conjunto de datos |
...
Utilice `fixed` para tablas de HTML que requieran imágenes equilibradas que se desequilibran cuando se selecciona `auto`. En este ejemplo, las imágenes tienen tamaños idénticos, pero hay más texto en la columna central.
...
<tr>
  <td>
    ...
    <em>Flujo de trabajo de edición principal para los redactores de posibles clientes.</em>
    <br>
  </td>
  <td>
    ...
    <em>¿No es escritor principal? Aprenda las formas más sencillas de realizar contribuciones. ¿No es un guionista? Aprenda las formas más sencillas de realizar contribuciones. ¿No es un guionista? Aprenda las formas más sencillas de realizar contribuciones. ¿No es un guionista? Aprenda las formas más sencillas de realizar contribuciones. ¿No es un guionista? Aprenda las formas más sencillas de realizar contribuciones. ¿No es un guionista? Conozca las formas más sencillas de hacer contribuciones.</em>
    <br>
  </td>
  <td>
    <a href="note-test.md">
